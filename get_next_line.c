...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/23 09:45:41 by fbock             #+#    #+#             */
/*   Updated: 2023/10/30 10:49:54 by fbock            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*fd_remainder;
	char		*result_string;

	if (fd < 0 || BUFFER_SIZE < 1 || fd > 1024)
		return (NULL);
	fd_remainder = read_and_alloc_buffer(fd, fd_remainder);
	if (!fd_remainder)
		return (NULL);
	result_string = cut_line(&fd_remainder);
	if (!result_string)
		return (NULL);
	if (ft_strlen(fd_remainder) == 0)
	{
		free(fd_remainder);
		fd_remainder = NULL;
	}
